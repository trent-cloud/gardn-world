...
Maintained by the `SESSION_END` protocol: each session-end prepends its block here and trims to the most recent three. This repo never built a per-session archive — only a rolling `SESSION_STATE.md` snapshot at the root. The most recent snapshot is included below as a single entry. Going forward, the canonical archive path is `mcleod/docs/sessions/YYYY-MM-DD.md`.
